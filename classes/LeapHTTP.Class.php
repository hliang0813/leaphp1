...
class LeapHTTP {
	...
	static public function init() {
		if (!is_object(self::$http)) {
			self::$http = new WP_Http();
		}
	}

	static public function request($url, $args = '') {
		...
		if (empty($args)) {
			...
			$response = self::$http->request($url, array(
				'method' => 'POST',
				'body' => $args,
			));
		}
		...
		if (isset($response->errors)) {
			throw new Exception($response->errors['http_request_failed'][0], 824209022);
		} else {
			if ($response['response']['code'] != 200) {
				throw new Exception('Get HTTP response code ' . $response['response']['code'] . '.', 824209023);
			} else {
	...
	static public function advance($url, $args = array()) {
		self::init();
		return self::$http->request($url, $args);
	}
}
